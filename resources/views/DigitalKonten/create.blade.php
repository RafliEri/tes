...
<head>
    ...
    <title>Digital Konten | Create Data</title>
...
<section class="bg-white flex items-center justify-center" style="height: 100vh; padding-bottom: 3rem">
  <div class="py-8 px-4 lg:py-16" style="width: 40vw;">
      ...
      <form action="{{ route('digitalkonten.store') }}" method="POST"
          enctype="multipart/form-data">
          @csrf
          <div class="grid gap-4 sm:grid-cols-2 sm:gap-6 pb-10">
              <div class="sm:col-span-2">
                  <label for="nama_konten" class="block mb-2 text-sm font-medium text-gray-900 ">Name Konten</label>
                  <input type="text" name="nama_konten" id="nama_konten" class="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full" placeholder="Ketik nama file" required="" style="background-color:#D9D9D9;">
              </div>
              <div class="sm:col-span-2">
          ...
          <button type="button" class="rounded-md text-black" aria-expanded="false" data-dropdown-toggle="dropdown-user" style="background-color:#D9D9D9;">
              <a href="{{ url('digitalkonten') }}" class="text-black-300 hover:bg-red-600 hover:text-white px-3 py-2 rounded-md text-sm font-medium flex">
                <svg class="size-3 opacity-75 mr-2 mt-1 w-3 h-3"
                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
